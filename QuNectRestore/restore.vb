...
Public Class frmRestore
    ...
    Private Const numberOfCommandLineArgs As Integer = 7
    ...
    Private Sub restore_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If cmbBulkorSingle.SelectedIndex = BulkOrSingle.TwoOrMore Then
            lblFileOrFolder.Text = GetSetting(AppName, "config", "folder", "")
        Else
            lblFileOrFolder.Text = GetSetting(AppName, "config", "file", "")
        End If
        ...
        If cmdLineArgs.Length = numberOfCommandLineArgs Then
            lblFileOrFolder.Text = cmdLineArgs(1)
...
#End If
    ...
    Private Function restoreTable(checkForErrorsOnly As Boolean, previewOnly As Boolean, bulkRestore As Boolean) As Boolean
        ...
        Dim m As Match = Regex.Match(lblFileOrFolder.Text, "\.csv$", RegexOptions.IgnoreCase)
        If Not m.Success Then
            PopUpMsgBox("Cannot restore from a non CSV file: " & lblFileOrFolder.Text, MsgBoxStyle.OkOnly, AppName)
            restoreTable = False
            Exit Function
        End If
        ...
        If BuiltInRecordIDisKey And importThese.Count > 0 Then
        ...
        Try
            Using connection As OdbcConnection = getquNectConn(connectionString)
                ...
                Using command As OdbcCommand = New OdbcCommand(strSQL, connection)
                    ...
                    Dim csvReader As New Microsoft.VisualBasic.FileIO.TextFieldParser(lblFileOrFolder.Text)
                        ...
                        While Not csvReader.EndOfData
                            ...
                            Catch ex As Exception
                                If checkForErrorsOnly Then
                                    ...
                                    End If
                                Else
                                    Throw New Exception("line " & fileLineCounter & " in '" & lblFileOrFolder.Text & "' has an issue. " & ex.Message)
                                End If
                            End Try
                            lineCounter += 1
    ...
    Private Sub lblFile_TextChanged(sender As Object, e As EventArgs) Handles lblFileOrFolder.TextChanged
        If cmbBulkorSingle.SelectedIndex = BulkOrSingle.TwoOrMore Then
            SaveSetting(AppName, "config", "folder", lblFileOrFolder.Text)
        Else
            SaveSetting(AppName, "config", "file", lblFileOrFolder.Text)
        End If
        showHideControls()
    End Sub
    ...
    Private Sub btnSource_Click(sender As Object, e As EventArgs) Handles btnSource.Click
        If cmbBulkorSingle.SelectedIndex = BulkOrSingle.TwoOrMore Then
            If (FolderBrowserDialog.ShowDialog() = DialogResult.OK) Then
                lblFileOrFolder.Text = FolderBrowserDialog.SelectedPath.ToString()
                SaveSetting(AppName, "config", "folder", lblFileOrFolder.Text)
    ...
    Private Sub OpenSourceFile_FileOk(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles OpenSourceFile.FileOk
        lblFileOrFolder.Text = OpenSourceFile.FileName.ToString()
        listFieldsAndReturnKeyFID(lblTable.Text, False)
    End Sub
    ...
    Function listFieldsAndReturnKeyFID(dbid As String, bulkRestore As Boolean) As String
        ...
        Try
            ...
            Using connection As OdbcConnection = getquNectConn(connectionString)
                ...
                End If
                Try
                    Dim fidReader As New Microsoft.VisualBasic.FileIO.TextFieldParser(Regex.Replace(lblFileOrFolder.Text, "\.csv$", ".fids"))
                    ...
                    Try
                        ...
                        For i As Integer = 0 To currentRow.Length - 1
                            ...
                            If sourceFieldNames.ContainsKey(sourceFieldName) Then
                                Throw New System.Exception("Duplicate field label '" & sourceFieldName & "' in CSV file: " & lblFileOrFolder.Text)
                            End If
    ...
    Private Sub btnImport_Click(sender As Object, e As EventArgs) Handles btnImport.Click
        Import(lblFileOrFolder.Text)
    End Sub
    Private Sub Import(fileOrFolder As String)
        If cmbBulkorSingle.SelectedIndex = BulkOrSingle.TwoOrMore Then
            Me.Cursor = Cursors.WaitCursor
            btnSource.Enabled = False
            cmbBulkorSingle.Enabled = False
            Application.DoEvents()
            Dim strFolder As String = fileOrFolder
            lblFileOrFolder.Text = strFolder
            Dim dbidToKeyFID As New Dictionary(Of String, String)
            Try
                Dim tableCounter As Integer = 0
                'import a whole directory
                Dim di As New DirectoryInfo(lblFileOrFolder.Text)
                ...
                For Each fri In fiArr
                    ...
                    lblFileOrFolder.Text = fri.FullName
